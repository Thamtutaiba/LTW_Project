...
router.put(
  ...
  async (req, res) => {
    ...
    const validStatus = ["pending", "processing", "shipping", "completed", "cancelled"];
...
// Quản lý vận chuyển - Lấy thông tin vận chuyển
router.get(
  "/orders/:id/shipping",
  authenticateToken,
  requireAdminRole,
  async (req, res) => {
    ...
    try {
      ...
      const [shippingInfo] = await connection.execute(
...
// Quản lý vận chuyển - Cập nhật thông tin vận chuyển
router.post(
  "/orders/:id/shipping",
  authenticateToken,
  requireAdminRole,
  async (req, res) => {
    const { id } = req.params;
    const { shipping_method, tracking_number, estimated_delivery, notes } = req.body;
    ...
    try {
      ...
      // Kiểm tra xem đơn hàng có ở trạng thái processing không
      const [orders] = await connection.execute(
      ...
      if (orders[0].status !== "processing") {
        ...
        return res.status(400).json({ 
          message: "Chỉ có thể cập nhật vận chuyển cho đơn hàng đang xử lý" 
        });
      }

      // Kiểm tra xem đã có thông tin vận chuyển chưa
      const [existingShipping] = await connection.execute(
        "SELECT id FROM shipping_info WHERE order_id = ?",
        [id]
      );
      ...
      if (existingShipping.length > 0) {
        ...
        await connection.execute(
          `UPDATE shipping_info SET 
           ...
           updated_at = NOW()
           WHERE order_id = ?`,
          [shipping_method, tracking_number, estimated_delivery, notes, id]
        );
      } else {
        // Tạo mới thông tin vận chuyển
        await connection.execute(
          `INSERT INTO shipping_info 
           (order_id, shipping_method, tracking_number, estimated_delivery, notes, status) 
           VALUES (?, ?, ?, ?, ?, 'in_transit')`,
          [id, shipping_method, tracking_number, estimated_delivery, notes]
        );
      }

      // Cập nhật trạng thái đơn hàng thành shipping
      await connection.execute(
        "UPDATE orders SET status = 'shipping', updated_at = NOW() WHERE id = ?",
        [id]
      );
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Kiểm tra xem đơn hàng có ở trạng thái shipping không
      const [orders] = await connection.execute(
        "SELECT status FROM orders WHERE id = ?",
        [id]
      );
      ...
      if (orders[0].status !== "shipping") {
        await connection.end();
        return res.status(400).json({ 
          message: "Chỉ có thể xác nhận giao hàng cho đơn hàng đang vận chuyển" 
        });
      }
      ...
      await connection.end();
      res.json({ 
        message: status === "delivered" ? "Xác nhận giao hàng thành công" : "Xác nhận giao hàng thất bại",
...
// Lấy danh sách đơn hàng với thông tin vận chuyển
router.get("/orders", authenticateToken, requireAdminRole, async (req, res) => {
  ...
  if (search) {
    where +=
      " AND (o.order_code LIKE ? OR o.shipping_name LIKE ? OR o.shipping_phone LIKE ?)";
  ...
  try {
    ...
    const [orders] = await connection.execute(
      `SELECT o.*, si.shipping_method, si.tracking_number, si.status as shipping_status, si.estimated_delivery, si.actual_delivery, si.notes as shipping_notes
       FROM orders o
